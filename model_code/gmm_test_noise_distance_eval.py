#!/usr/bin/env python3
# gmm_test_noise_distance_eval.py
#
# GMM-based NLL "distance from training distribution" analysis on the
# TEST split, across 7 conditions:
#   plain, awgn_15dB, awgn_5dB, awgn_-5dB, clutter_15dB, clutter_5dB, clutter_-5dB
#
# For each condition and each k in {1..MAX_K} (plus per-position optimal k):
#   - Computes per-sample NLL distance map [128, 128, 31] under the
#     TRAIN-fitted 1x1x1 GMMs (gmm_scalar_fit_1x1x1.py / gmm_scalar_aggregate_1x1x1.py)
#   - Summarizes statistics (mean, std, quantiles) over the map
#   - Saves per-sample CSV and dataset-level summary CSV per condition/k
#
# This is the test-set counterpart of gmm_scalar_distance_eval.py (which
# covers the val "evaluation" split). It also fixes a path mismatch in
# that script: it looked for GMM_MODEL_ROOT/k{k}/weights.pt and
# GMM_MODEL_ROOT/optimal_k_map.pt, but gmm_scalar_aggregate_1x1x1.py
# actually writes weights_k{k:02d}.pt / optimal_k.pt into a separate
# aggregate directory. Requires gmm_scalar_aggregate_1x1x1.py to have
# been run first.
#
# Outputs go to ./gmm_test_noise_eval_results/
...
FEATURE_DIRS = {c: CSV_ROOT / c / "features" / "model" for c in CONDITIONS}

GMM_MODEL_ROOT = Path("./gmm_scalar_models_1x1x1")
GMM_AGG_DIR    = Path("./gmm_scalar_aggregate_1x1x1")  # matches gmm_scalar_aggregate_1x1x1.py
SAVE_ROOT      = Path("./gmm_test_noise_eval_results")
...
def compute_nll(x, weights, means, variances):
    """
    x: (H, W, T); weights/means/variances: (H, W, T, k)
    Returns nll: (H, W, T)
    """
    x_e       = x.unsqueeze(-1)
    diff      = x_e - means
...
def save_dataset_summary(result_df, condition, k_label, save_root):
    ...
    pd.DataFrame([summary]).T.to_csv(save_root / f"{condition}_k{k_label}_summary.csv")
...
print(f"\nTest rows (is_corrupt==False & sample_9600==True): {len(test_df)}")

# =========================================================
# STEP 3: EVALUATE PER CONDITION
# =========================================================
for condition in CONDITIONS:
    feature_dir = FEATURE_DIRS[condition]
    print(f"\nProcessing condition: {condition}  ({feature_dir})")

    if not feature_dir.exists():
        print(f"  [WARNING] Feature dir does not exist, skipping: {feature_dir}")
        continue

    rows = []
    for _, meta_row in tqdm(test_df.iterrows(), total=len(test_df), desc=condition):
        idx = str(int(float(meta_row["ID"])))
        idx = f"{int(idx):08d}"
