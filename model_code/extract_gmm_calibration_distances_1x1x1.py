#!/usr/bin/env python3
# extract_gmm_calibration_distances_1x1x1.py
#
# Uses fitted GMM models (from gmm_scalar_fit_1x1x1.py) to compute per-sample
# NLL distance maps for the calibration split of the val set (plain condition).
#
...
#       .../plain/distances/model/gmm_k{k}/{sample_id}.pt
#
# Also computes distances using the per-position optimal k (from optimal_k_map.pt):
#   - Saves to: .../plain/distances/model/gmm_optimal_k/{sample_id}.pt
#
# These distance tensors will be consumed by the conformal calibration script later.
...
# Matches gmm_scalar_aggregate_1x1x1.py's AGG_OUT_DIR (kept local so it
# survives the scratch4weeks retention purge; see that script's note).
GMM_AGG_DIR    = Path("./gmm_scalar_aggregate_1x1x1")
...
summary_df = pd.DataFrame([{
    ...
    "max_k":                MAX_K,
    "gmm_model_root":       str(GMM_MODEL_ROOT.resolve()),
