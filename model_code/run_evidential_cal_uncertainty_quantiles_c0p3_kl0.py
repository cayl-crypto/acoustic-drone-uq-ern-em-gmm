#!/usr/bin/env python3
# run_evidential_cal_uncertainty_quantiles_c0p3_kl0.py
#
# Runs best_evidential_loc_logmel_plain_plain_c0p3_kl0.pt (coeff=0.3,
# kl_weight=0.0) on the full calibration set (N=2912), computing native
# NIG evidential uncertainty (not the GMM feature-density approach) and its
# conformal quantiles. Mirrors run_evidential_cal_uncertainty_quantiles.py
# (which was run against the old best_evidential_model.pt, coeff=0.1) --
# retargeted at the new checkpoint since coeff/kl_weight shape how
# confident the model's own evidential outputs are, so the two
# checkpoints' uncertainty calibrations aren't interchangeable.
#
# All 2912 calibration IDs are loaded from the plain logmel feature folder,
# regardless of the noise_level column in the CSV (every ID, no filter).
#
# Outputs (all under ./evidential_calibration_results_c0p3_kl0/):
#   plain_calibration_set_per_sample.csv
#   plain_calibration_set_summary.csv
#   plain_calibration_set_uncertainty_quantiles.csv   (13 scalars x min/q6827/q9545/q9973/max/mean/std)
#   plain_calibration_set_uncertainty_quantiles.pt    (same as dict, keyed by scalar name)
#
# Next: apply_evidential_conformal_bands_to_test_c0p3_kl0.py
...
SAVE_DIR     = Path("./evidential_calibration_results_c0p3_kl0")
...
# 4 per head (aleatoric_var, epistemic_var, total_var, sigma) x 3 heads + bearing_std_deg
# =========================================================
uncertainty_cols = [
...
print("Next: run apply_evidential_conformal_bands_to_test_c0p3_kl0.py")
