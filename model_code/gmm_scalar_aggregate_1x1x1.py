#!/usr/bin/env python3
# gmm_scalar_aggregate_1x1x1.py
#
# Run after all 8 gmm_scalar_fit_1x1x1.py jobs finish.
# Reads per-position .pt files from GMM_MODEL_ROOT/positions/
# and builds dense (H, W, T) tensors for weights, means, variances,
# log-likelihoods, BICs, and optimal k.
#
# Output layout inside GMM_MODEL_ROOT/:
...
#
# Usage:
#   python gmm_scalar_aggregate_1x1x1.py
...
# NOTE: previously pointed at scratch4weeks (.../plain/distances/model/gmm_scalar_aggregate_1x1x1)
# and got wiped by the scratch4weeks retention purge before extract_gmm_calibration_distances_1x1x1.py
# ever consumed it. Kept local now so it survives; only the per-position source files
# (GMM_MODEL_ROOT/positions/, already local) are read to build it.
AGG_OUT_DIR = Path("./gmm_scalar_aggregate_1x1x1")
...
print("GMM scalar aggregate — 1x1x1")
print(f"  Source : {POSITION_DIR}")
print(f"  Output : {GMM_MODEL_ROOT}")
print()
...
print("Next: run extract_gmm_calibration_distances_1x1x1.py")
